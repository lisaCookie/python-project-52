...
class LabelTests(TestCase):
    ...
    def test_label_update_view_post_success(self):
        ...
        response = self.client.post(reverse('label_update', args=[self.label.pk]), data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('label_list'))
    ...
    def test_label_update_view_post_duplicate(self):
        ...
        self.client.login(username='testuser', password='testpass123')
        data = {'name': 'Test Label'}  # Пытаемся переименовать в существующее имя
        response = self.client.post(reverse('label_update', args=[other_label.pk]), data)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Метка с таким именем уже существует')
    ...
    def test_label_delete_view_post_success(self):
        ...
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(reverse('label_delete', args=[deletable_label.pk]))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('label_list'))
    ...
    def test_label_delete_view_post_with_tasks(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('label_list'))
        self.assertTrue(Label.objects.filter(pk=self.label.pk).exists())  # Метка не удалена
    ...
    def test_label_form_duplicate_validation(self):
        ...
        form = LabelForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['name'][0], 'Метка с таким именем уже существует')
    ...
    def test_label_form_duplicate_validation_self_update(self):
        ...
        self.assertTrue(form.is_valid())  # Должно быть валидно при обновлении себя
