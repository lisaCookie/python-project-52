...
class TaskTests(TestCase):
    def setUp(self):
        ...
        self.other_user = User.objects.create_user(
            username="otheruser", password="otherpass123"   # NOSONAR
        )
    ...
    def test_task_list_view_filters(self):
        ...
        response = self.client.get(
            f"{reverse('task-list')}?status={self.status.id}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Task")
        ...
        response = self.client.get(
            f"{reverse('task-list')}?executor={self.user.id}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Task")
        ...
        # Test label filter
        response = self.client.get(
            f"{reverse('task-list')}?label={self.label.id}"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Test Task")
    ...
    def test_task_update_view_post_success(self):
        ...
        response = self.client.post(
            reverse("task-update", args=[self.task.pk]), data
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("task-list"))
    ...
    def test_success_messages(self):
        ...
        response = self.client.post(
            reverse("task-update", args=[self.task.pk]), data, follow=True
        )
        messages_list = list(get_messages(response.wsgi_request))
        self.assertIn(
            "Задача успешно изменена", [str(m) for m in messages_list]
        )

        # Test delete message
        response = self.client.post(
            reverse("task-delete", args=[self.task.pk]), follow=True
        )
        messages_list = list(get_messages(response.wsgi_request))
