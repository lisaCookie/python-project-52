...
class StatusTests(TestCase):
    ...
    def test_status_update_view_post_success(self):
        ...
        response = self.client.post(reverse('status-update', args=[self.status.pk]), data)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('status-list'))
    ...
    def test_status_delete_view_get(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.get(reverse('status-delete', args=[self.status.pk]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_status_delete_view_post_success(self):
        ...
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(reverse('status-delete', args=[deletable_status.pk]))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('status-list'))
    ...
    def test_status_delete_view_post_with_tasks(self):
        self.client.login(username='testuser', password='testpass123')
        response = self.client.post(reverse('status-delete', args=[self.status.pk]))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('status-list'))
    ...
    def test_status_form_duplicate_validation(self):
        ...
        form = StatusForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['name'][0], 'Статус с таким именем уже существует')
